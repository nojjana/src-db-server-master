...
export class WhackAMoleProgram implements Program {
    ...
    private setDisplayWhackBuildListener(): void {
        let displays = this.lobbyController.getDisplays();

        for(let display of displays){
            display.addSocketOnce('whackAMoleBuild', this.whackAMoleBuild.bind(this));
        }
    }

    private whackAMoleBuild(): void {
        this.readyDisplays++;
    ...
    private sendLevelInfoToDisplay(): void {
        ...
        this.setDisplayWhackBuildListener();
        this.lobbyController.sendToDisplays('whackData', data);
    }
